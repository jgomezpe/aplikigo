	...
	public Token getTokenList(Segment text, int startTokenType, int startOffset) {
		...
		if( input != null && input.length()>0 ){
			...
			for( nsgl.language.Token t:token ){
				int start = t.location() + offset;
				if( start>currentTokenStart ) addToken(array, currentTokenStart,start-1, Token.ERROR_IDENTIFIER, newStartOffset+currentTokenStart);
		...
		if( currentTokenStart-offset < count ) addToken(array, currentTokenStart,offset+count-1, Token.ERROR_IDENTIFIER, newStartOffset+currentTokenStart);
		addNullToken();
		return firstToken;
	}
